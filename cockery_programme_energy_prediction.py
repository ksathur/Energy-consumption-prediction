...
plt.title('Cockery Energy Consumption(Actual)')
...
# plt.plot(data)
plt.savefig('dumps/accuracy/cockery_actual.png')
plt.show()

#################################################
	# Data Preprocessing
#################################################
# all_data = data.values.astype(float)
test_data_size = 7
...
plt.title('Cockery Energy Consumption')
plt.ylabel('Energy Consumption (kWh)')
plt.xlabel('Day')
plt.grid(True)
# plt.autoscale(axis='x', tight=True)
...
plt.gca().legend()
plt.savefig('dumps/accuracy/cockery_actual_and_predicted.png')
plt.show()
...
plt.plot(x, actual_predictions, label = 'Predicted')
plt.gca().legend()
plt.savefig('dumps/accuracy/cockery_actual_and_predicted_zoom.png')
plt.show()

################################################
	# Dumping files
################################################
file_name = 'dumps/array/cockery_inputs_and_outputs.npz'
actual_data = data
